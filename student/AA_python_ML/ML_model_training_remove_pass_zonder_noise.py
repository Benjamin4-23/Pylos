...
def train_dqn(data_file: str, num_epochs: int = 100, 
    ...
    BASE_Q = -5.0 # Base Q-value for non-dataset moves
    MARGIN = 2.0  # Minimum margin between dataset and non-dataset moves
    REWARD_SCALE = 1.0 
...
if __name__ == "__main__":
    start_epoch = 386
    num_epochs = 501
    load_path = "checkpoint_epoch_385_RP.pth"

    model, history = train_dqn("data_shuffled_remove_pass.csv", 
                        num_epochs=num_epochs, #,  # Train until epoch 100
                        start_epoch=start_epoch,   # Start from epoch 1
                        load_path=load_path)  # Load previous state
    plot_training_metrics(history)
    cleanup_between_datasets()
    # start_epoch += 100
    # num_epochs += 100
    # load_path = f"checkpoint_epoch_{start_epoch-1}.pth"
    # model, history = train_dqn("data_subset_1.csv", 
    #                     num_epochs=num_epochs,  # Train until epoch 100
    #                     start_epoch=start_epoch,   # Start from epoch 1
    #                     load_path=load_path)  # Load previous state
    # plot_training_metrics(history)
    ...
    # num_epochs += 100
    # load_path = f"checkpoint_epoch_{start_epoch-1}.pth"
    # model, history = train_dqn("data_subset_2.csv", 
    #                     num_epochs=num_epochs,  # Train until epoch 100
    #                     start_epoch=start_epoch,   # Start from epoch 1
    #                     load_path=load_path)  # Load previous state
    ...
    # plot_training_metrics(history)
    export_model_to_onnx(model, "pylos_model_remove_pass_final.onnx")
